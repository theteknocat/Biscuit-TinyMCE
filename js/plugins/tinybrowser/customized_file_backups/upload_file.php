...
if ($_FILES['Filedata']['tmp_name'] && $_FILES['Filedata']['name'])
	{	
		...
		if($success && !isset($_GET['standard_upload_form']))
			{
	...
	if (isset($_GET['standard_upload_form']) && $_GET['standard_upload_form']) {
		Console::log("Composing upload response from standard upload form...");
		if ($_FILES['Filedata']['tmp_name'] && $_FILES['Filedata']['name']) {
			Console::log("Files were uploaded successfully");
			$filetotal = 1;
			$file_errors = 0;
		} else {
			Console::log("Files did not upload");
			if ($_FILES['Filedata']['error']) {
				$file_errors = 1;
				Console::log("Upload error: ".$_FILES['Filedata']['error']);
			}
			$filetotal = 0;
		}
		header('Location: ./upload_process.php?standard_upload_form=1&type='.$_GET['type'].'&feid='.$_GET['feid'].'&folder='.$_GET['folder'].'&filetotal='.$filetotal.'&file_errors='.$file_errors);
	}
?>
